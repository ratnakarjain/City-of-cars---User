...
timedifference(String time) {
  ...
  String retTime;
  var difference = date2.difference(DateTime.parse(time)).inSeconds;
  retTime = difference.toString() + " seconds ago";
  if (difference > 60) {
    var difference = date2.difference(DateTime.parse(time)).inMinutes;
    retTime = difference.toString() + " minutes ago";

    if (difference > 60) {
      var difference = date2.difference(DateTime.parse(time)).inHours;
      retTime = difference.toString() + " hours ago";
      if (difference > 24) {
        retTime = time.toString().substring(0, 10);
      }
    }
  }
  return retTime;
